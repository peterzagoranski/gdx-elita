...
public class PagedScrollPane extends ScrollPane {
    ...
    private void scrollToPage () {
        ...
        if (pages.size > 0) {
            ...
            float clampX = MathUtils.clamp(pageX - (width - pageWidth) / 2, 0, maxX);
            setScrollX(clampX);

            if(null != this.mPageChangedListener) {
                this.mPageChangedListener.onPageChanged((int)(clampX/width));
            }
        }
    }

    public interface IPageChangedListener {
        void onPageChanged(final int pIndex);
    }

    public void addPageChangedListener(final IPageChangedListener pListener) {
        this.mPageChangedListener = pListener;
    }
}